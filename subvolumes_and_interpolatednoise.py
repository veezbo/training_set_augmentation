...
def applyInterpolatedNoise(image, target, noise_factor=10, stride=256):
    ...
    return bilinear_interpolate(image, x_jitter, y_jitter), lookupNearest(target, x_jitter, y_jitter)
...
def applyInterpolatedNoiseToStack(images, targets, noise_factor=10, stride=256):
    ...
    for s in range(len(images)):
        data_noise[s], label_noise[s] = applyInterpolatedNoise(images[s], target[s])
